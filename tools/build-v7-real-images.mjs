...
(() => {
  ...
  function enhancePractice(){document.querySelectorAll('.practice-card:not([data-photo-enhanced])').forEach(card=>{const title=card.querySelector('h3,b,strong')?.textContent||card.textContent;const photo=figure(findByContainedText(DATA.practice,title),String(title).trim(),true);if(photo){const diagram=card.querySelector('.practice-diagram');diagram?diagram.before(photo):card.insertBefore(photo,card.children[1]||null);card.dataset.photoEnhanced='1';}});}
